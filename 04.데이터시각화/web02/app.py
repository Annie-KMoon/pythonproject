...
def graph2():
    x=df.index*4
    plt.figure(figsize=(10,5))
    plt.bar(x+0, df['국어'], label='국어', width=1,color='#FFFAFA')
    plt.bar(x+1, df['영어'], label='영어', width=1,color='#F8F8FF')
    plt.bar(x+2, df['수학'], label='수학', width=1, color='#F0FFF0')
    plt.legend(loc=(1.02,0.1))
...
def graph4():
    ...
    plt.scatter(df['국어'],df['영어'],sizes=df['학년']*500,
                c = df['학년'], cmap='autumn_r', alpha=0.3)
    plt.xlabel('국어')
    plt.ylabel('영어')
    ...
    for idx in range(len(df)):
        ...
        y=df.loc[idx,'영어']
        name=df.loc[idx,'이름']
        plt.text(x,y,name, size=6, ha='center')
